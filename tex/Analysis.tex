...
\begin{figure}[h]
	...
	\caption{A comparison of all the cuts, where \note{with the energy...} the energy of the first \al-particle is plotted. }
...
\begin{figure}[h]
	...
	\label{fig:oneDetEff}
\end{figure}
In this setup however, we \note{mangler et ord her} a cube of square detectors, who's normal vectors are all pointing in towards target at the center. This gives a much larger coverage of all mutual angles. 
...
\begin{figure}[h]
	...
	\label{fig:effAllDet}
\end{figure}
There is still a geometric effect that is not accounted for in the above analysis. We  \note{måske slet still - gentagelse}still need to consider that not all pixels in the detector has the same effective area. 
...
To account for this effect \note{det lyder som om der referes til effekten af de døde strips}, each pixel will be associated with a corresponding area-efficiency $(\text{Eff}_A)$.
...
\begin{figure}[H]
	...
	\caption{A illustration of two hits in different pixels. The blue hit will see a smaller effective area of the pixel by a factor determined by \cref{eq:eff} \note{Måske tilføj "den blå retnmingsvektor projiceret på den stiplede linje bliver minder for større theta eller noget´}. \note{Denne figur er kommet i forkert rækkefølge}}
...
This is done by first finding the angle between the first \al-particle \al$_1$ \note{vinkel mellem alfa og hvad?}, and create a histogram of this. Then the angle between the second \al-particle \al$_2$ is measured \note{vinkel mellem alfa og hvad?}, and a histogram is created. The two histograms are then added, to get the full picture of the mutual angles of the particles. 
